...
function WriteExam() {
  ...
  const calculateResult = async () => {
    try {
      ...
      dispatch(HideLoading());
      if (response.success) {
        // Identify weaker sections and fetch YouTube recommendations
        const weakTopics = getWeakerSections(wrongAnswers);
        await fetchYoutubeRecommendations(weakTopics);
        setView("result");
      } else {
        message.error(response.message);
      }
  ...
  // Function to determine weak sections
  const getWeakerSections = (wrongAnswers) => {
    const weakTopics = new Set();
    wrongAnswers.forEach((question) => {
      // Assume question.topic exists; you can customize this based on your question model
      weakTopics.add(question.topic || "General Knowledge");
    });
    return Array.from(weakTopics);
  };

  // Fetch YouTube recommendations based on weak topics
  const fetchYoutubeRecommendations = async (topics) => {
    ...
    try {
      ...
      message.error("Failed to fetch YouTube recommendations");
    }
  };
